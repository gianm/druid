...
public class ServerManager implements QuerySegmentWalker
{
  ...
  public <T> QueryRunner<T> getQueryRunnerForIntervals(Query<T> query, Iterable<Interval> intervals)
  {
    ...
    final VersionedIntervalTimeline<String, ReferenceCountingSegment> timeline = segmentManager.getTimeline(
        ((TableDataSource) analysis.getBaseDataSource()).getName()
    );

    if (timeline == null) {
      // TODO(gianm): Not correct when there's a right join going on!
  ...
  public <T> QueryRunner<T> getQueryRunnerForSegments(Query<T> query, Iterable<SegmentDescriptor> specs)
  {
    ...
    // segmentMapFn potentially maps each base Segment into a joined Segment.
    // Accumulate the CPU time metric while doing this, since it can potentially take some time, and we want to reflect
    // that (e.g. hash tables must be prepared if not already precomputed).
    // TODO(gianm): Add specific metrics for this.
    final Function<Segment, Segment> segmentMapFn = JvmUtils.safeAccumulateThreadCpuTime(
        cpuTimeAccumulator,
        () -> analysis.getBaseSegmentMapFn(joinableFactory)
    );
