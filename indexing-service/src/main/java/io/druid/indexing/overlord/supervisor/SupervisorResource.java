...
/**
 * Endpoints for submitting a {@link SupervisorSpec}, getting running supervisors, and stopping supervisors.
 */
            ...
            if (manager.hasSupervisor(spec.getId())) {
              return Response.status(Response.Status.CONFLICT).entity(
                  ImmutableMap.of(
                      "error",
                      String.format(
                          "Supervisor already exists for [%s]",
                          spec.getId()
                      )
                  )
              ).build();
  ...
  public Response specGetAll()
  {
    return asLeaderWithSupervisorManager(
        new Function<SupervisorManager, Response>()
        {
          ...
          public Response apply(SupervisorManager manager)
          {
            return Response.ok(manager.getSupervisors()).build();
          }
        }
    );
  }
