  ...
  )
  {
    ...
    if (kind == SqlKind.INPUT_REF) {
      ...
      if (rexNode.getType().getSqlTypeName() == SqlTypeName.DATE
        ...
        final PeriodGranularity granularity = TimeUnits.toQueryGranularity(
            TimeUnitRange.DAY,
        ...
        return TimeFloorOperatorConversion.applyTimestampFloor(operandExpression, granularity);
      } else {
        final ExprType fromType = EXPRESSION_TYPES.get(operand.getType().getSqlTypeName());
        final ExprType toType = EXPRESSION_TYPES.get(sqlTypeName);
        ...
        if (fromType != toType) {
          return operandExpression.map(
              Function.identity(), // Ignore casts for simple extractions.
              expression -> String.format("CAST(%s, '%s')", expression, toType.toString())
          );
        } else {
