...
/**
 * DruidRel that uses a "equiJoin" dataSource.
 */
public class DruidJoinQueryRel extends DruidRel<DruidJoinQueryRel>
{
  ...
  private final boolean leftIsScanOrMapping;
  private final boolean rightIsScanOrMapping;

  private DruidJoinQueryRel(
      RelOptCluster cluster,
      RelTraitSet traitSet,
      Join joinRel,
      boolean leftIsScanOrMapping,
      boolean rightIsScanOrMapping,
  ...
  )
  {
    ...
    this.leftIsScanOrMapping = leftIsScanOrMapping;
    this.rightIsScanOrMapping = rightIsScanOrMapping;
    this.partialQuery = partialQuery;
  }

  public static DruidJoinQueryRel create(
      final Join joinRel,
      final boolean leftIsScanOrMapping,
      final boolean rightIsScanOrMapping,
  ...
  public DruidQuery toDruidQuery(final boolean finalizeAggregations)
  {
    ...
    if (DruidJoinRule.isScanOrMapping(leftDruidRel, true)) {
      leftDataSource = leftQuery.getDataSource();
    } else {
      leftDataSource = new QueryDataSource(leftQuery.getQuery());
    }

    if (DruidJoinRule.isScanOrMapping(rightDruidRel, false)) {
  ...
  {
    // TODO(gianm): Update cost function to (A) use DruidQueryRel's stuff
    // TODO(gianm): Are we figuring out "is table scan" properly?
    // TODO(gianm): Extract constants somewhere useful.
    return planner.getCostFactory()
                  .makeCost(mq.getRowCount(left), 0, 0)
                  .plus(planner.getCostFactory().makeCost(mq.getRowCount(right), 0, 0))
                  .multiplyBy(leftIsScanOrMapping ? 1000.0 : 1000000000.0)
                  .multiplyBy(rightIsScanOrMapping ? 1.0 : 100000.0);
  }
