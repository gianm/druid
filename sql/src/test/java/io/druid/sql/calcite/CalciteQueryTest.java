...
public class CalciteQueryTest
{
  ...
  @Test
  public void testLotsOfFunctions() throws Exception
  {
    // The row we're getting:
    //   ImmutableMap.of("t", "2000-01-02", "m1", "2.0", "dim1", "10.1", "dim2", ImmutableList.of()))

    final List<String> expectedSqls = new ArrayList<>();
    final List<String> expectedExps = new ArrayList<>();
    final List<ValueType> expectedTyps = new ArrayList<>();
    final List<Object> expectedOuts = new ArrayList<>();

    expectedSqls.add("dim1 || ' ' || CAST(m1 AS VARCHAR)");
    expectedExps.add("concat(concat(\"dim1\",' '),CAST(\"m1\", 'STRING'))");
    expectedTyps.add(ValueType.STRING);
    expectedOuts.add("10.1 2.0");

    expectedSqls.add("FLOOR(__time TO DAY)");
    expectedExps.add("timestamp_floor(\"__time\",'P1D','','UTC')");
    expectedTyps.add(ValueType.LONG);
    expectedOuts.add(T("2000-01-02T00Z"));

    expectedSqls.add("CEIL(__time TO DAY)");
    expectedExps.add("timestamp_ceil(\"__time\",'P1D','','UTC')");
    expectedTyps.add(ValueType.LONG);
    expectedOuts.add(T("2000-01-03T00Z"));

    expectedSqls.add("COALESCE(NULLIF(m1, 2), cnt)");
    expectedExps.add("case_searched((\"m1\" != 2),case_searched((\"m1\" == 2),'',\"m1\"),CAST(\"cnt\", 'DOUBLE'))");
    expectedTyps.add(ValueType.FLOAT);
    expectedOuts.add(1.0d);

    expectedSqls.add("COALESCE(CASE WHEN m1 = 1 THEN 'foo' END, 'bar')");
    expectedExps.add("case_searched((\"m1\" == 1),case_searched((\"m1\" == 1),'foo',''),'bar')");
    expectedTyps.add(ValueType.STRING);
    expectedOuts.add("bar");

    expectedSqls.add("COALESCE(CASE WHEN m1 = 2 THEN 'foo' END, 'bar')");
    expectedExps.add("case_searched((\"m1\" == 2),case_searched((\"m1\" == 2),'foo',''),'bar')");
    expectedTyps.add(ValueType.STRING);
    expectedOuts.add("foo");

    expectedSqls.add("CAST(cnt AS FLOAT) + 6 - 2.0 / 4");
    expectedExps.add("((CAST(\"cnt\", 'DOUBLE') + 6) - 0.5)");
    expectedTyps.add(ValueType.FLOAT);
    expectedOuts.add(6.5d);

    expectedSqls.add("FLOOR(m1 * 2.2)");
    expectedExps.add("floor((\"m1\" * 2.2))");
    expectedTyps.add(ValueType.FLOAT);
    expectedOuts.add(4.0d);

    expectedSqls.add("CEIL(m1 * 2.2)");
    expectedExps.add("ceil((\"m1\" * 2.2))");
    expectedTyps.add(ValueType.FLOAT);
    expectedOuts.add(5.0d);

    expectedSqls.add("CASE WHEN dim1 LIKE '%0.%' THEN 'y' ELSE 'n' END");
    expectedExps.add("case_searched(like(\"dim1\",'%0.%'),'y','n')");
    expectedTyps.add(ValueType.STRING);
    expectedOuts.add("y");

    expectedSqls.add("CASE WHEN dim1 LIKE '%x0.%' ESCAPE 'x' THEN 'y' ELSE 'n' END");
    expectedExps.add("case_searched(like(\"dim1\",'%x0.%','x'),'y','n')");
    expectedTyps.add(ValueType.STRING);
    expectedOuts.add("y");
    ...
    expectedSqls.add("TIME_FORMAT(__time)");
    expectedExps.add("timestamp_format(\"__time\",'yyyy-MM-dd\\u0027T\\u0027HH:mm:ss.SSSZZ','UTC')");
    expectedTyps.add(ValueType.STRING);
    expectedOuts.add("2000-01-02T00:00:00.000+00:00");

    expectedSqls.add("TIME_FORMAT(__time, 'yyyy MM')");
    expectedExps.add("timestamp_format(\"__time\",'yyyy MM','UTC')");
    expectedTyps.add(ValueType.STRING);
    expectedOuts.add("2000 01");

    expectedSqls.add("TIME_FORMAT(__time, 'yyyy MM dd HH', 'America/Los_Angeles')");
    expectedExps.add("timestamp_format(\"__time\",'yyyy MM dd HH','America/Los_Angeles')");
    expectedTyps.add(ValueType.STRING);
    expectedOuts.add("2000 01 01 16");

    expectedSqls.add(
        "COALESCE(TIME_PARSE('2000 01 01 ' || CAST(cnt * 18 AS VARCHAR), 'yyyy MM dd HH'), TIMESTAMP '1990-01-01 00:00:00')");
    expectedExps.add("case_searched("
                     + "(timestamp_parse(concat('2000 01 01 ',CAST((\"cnt\" * 18), 'STRING')),'yyyy MM dd HH') != ''),"
                     + "timestamp_parse(concat('2000 01 01 ',CAST((\"cnt\" * 18), 'STRING')),'yyyy MM dd HH'),"
                     + "631152000000)");
    expectedTyps.add(ValueType.LONG);
    expectedOuts.add(T("2000-01-01T18"));

    expectedSqls.add(
        "COALESCE(TIME_PARSE('2000 01 z01 ' || CAST(cnt * 18 AS VARCHAR), 'yyyy MM dd HH'), TIMESTAMP '1990-01-01 00:00:00')");
    expectedExps.add("case_searched("
                     + "(timestamp_parse(concat('2000 01 z01 ',CAST((\"cnt\" * 18), 'STRING')),'yyyy MM dd HH') != ''),"
                     + "timestamp_parse(concat('2000 01 z01 ',CAST((\"cnt\" * 18), 'STRING')),'yyyy MM dd HH'),"
                     + "631152000000)");
    expectedTyps.add(ValueType.LONG);
    expectedOuts.add(T("1990-01-01T00"));

    final List<VirtualColumn> expectedVirtualColumns = new ArrayList<>();
    for (int i = 0; i < expectedSqls.size(); i++) {
      expectedVirtualColumns.add(EXPRESSION_VIRTUAL_COLUMN("v" + i, expectedExps.get(i), expectedTyps.get(i)));
    }
    ...
    testQuery(
        ...
        ImmutableList.of(
            Druids.newSelectQueryBuilder()
                  ...
                  .intervals(QSS(Filtration.eternity()))
                  .granularity(Granularities.ALL)
                  .filters(NOT(SELECTOR("m1", "1", null)))
                  .dimensions(ImmutableList.of("dummy"))
                  .metrics(expectedVirtualColumns.stream()
        ...
        ImmutableList.of(
            expectedOuts.toArray(new Object[]{})
        )
    );
  }
  ...
  public void testFilteredAggregations() throws Exception
  {
    testQuery(
        ...
        + "COUNT(dim2) filter(WHERE dim2 IS NOT NULL)"
        + "FROM druid.foo",
        ImmutableList.of(
            Druids.newTimeseriesQueryBuilder()
                  .dataSource(CalciteTests.DATASOURCE1)
                  ...
                  .aggregators(AGGS(
                      ...
                      new FilteredAggregatorFactory(
                          new CountAggregatorFactory("a10"),
        ...
        ImmutableList.of(
            new Object[]{1L, 5L, 1L, 5L, 5L, 5, 2L, 1L, 5L, 1L, 3L}
        )
    );
  }
