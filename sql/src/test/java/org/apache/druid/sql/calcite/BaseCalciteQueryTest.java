...
public class BaseCalciteQueryTest extends CalciteTestBase
{
   ...
   */
  private <T> Query<T> overrideAllQueryContexts(final Query<T> query, final Map<String, Object> context)
  {
    return query.withDataSource(overrideAllQueryContexts(query.getDataSource(), context))
   ...
   */
  private DataSource overrideAllQueryContexts(final DataSource dataSource, final Map<String, Object> context)
  {
    if (dataSource instanceof QueryDataSource) {
      final Query subquery = ((QueryDataSource) dataSource).getQuery();
      return new QueryDataSource(overrideAllQueryContexts(subquery, context));
    } else {
      return dataSource.withChildren(
          dataSource.getChildren()
                    .stream()
                    .map(ds -> overrideAllQueryContexts(ds, context))
  ...
  ) throws Exception
  {
    ...
    for (final String vectorize : vectorizeValues) {
      ...
      for (Query query : expectedQueries) {
        theQueries.add(overrideAllQueryContexts(query, theQueryContext));
      }
