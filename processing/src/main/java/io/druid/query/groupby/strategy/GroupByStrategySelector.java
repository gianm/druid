...
public class GroupByStrategySelector
{
  ...
  private final EpiGroupByStrategy epi;

  @Inject
  public GroupByStrategySelector(
      Supplier<GroupByQueryConfig> configSupplier,
      OldFaithfulGroupByStrategy oldFaithful,
      EpiGroupByStrategy epi
  )
  {
    this.config = configSupplier.get();
    this.oldFaithful = oldFaithful;
    this.epi = epi;
  }
  ...
  public GroupByStrategy strategize(GroupByQuery query)
  {
    ...
    switch (strategyString) {
      case STRATEGY_EPINEPHELINAE:
        return epi;

      case STRATEGY_OLD_FAITHFUL:
        return oldFaithful;
